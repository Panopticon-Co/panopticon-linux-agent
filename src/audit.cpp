...
result<bool> append_audit_record(const std::filesystem::path& path, const std::string_view action,
                                 ...
                                 const std::size_t maximum_record_bytes, const std::size_t maximum_file_bytes) {
    if (!is_valid_identifier(action) || !is_valid_identifier(correlation_id) || !is_valid_identifier(outcome) || maximum_record_bytes == 0U || maximum_file_bytes == 0U)
    ...
    const auto current_size = std::filesystem::exists(path, ec) ? std::filesystem::file_size(path, ec) : 0U;
    if (ec) return error{error_code::io_failure, "cannot inspect audit storage"};
    if (line.size() > maximum_file_bytes || current_size > maximum_file_bytes - line.size()) return error{error_code::resource_limit, "audit storage quota exceeded"};
